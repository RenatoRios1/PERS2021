...
dependencies = {'mcor': mcor, 'precision': precision, 'recall': recall, 'f1': f1, 'auc': auc}
if loadmodel==True:
    print("loading model")
    loaded_model = load_model('OneChannelnmn_oversampled_deepchanel6_5.h5', 
                              custom_objects= dependencies)#{'mcor': mcor, 'precision': precision, 'recall': recall, 'f1': f1, 'auc': auc}) 
    print("model loaded")
root = filedialog.Tk()
...
for file2open in filenames:
	df30 = pd.read_csv(file2open,header=1)
	dataset=df30.values
