...
Qubname = 'outfinaltest747_SKM.csv'
Qub2name = 'outfinaltest747_halfamp.csv'
Dname = 'outfinaltest747.csv'
...
loaded_model = load_model('Run1_T1_V2_M747.h5', custom_objects={
                          'mcor': mcor, 'precision': precision, 'recall': recall, 'f1Score': f1Score, 'auc': auc})
...
##START STUFF RENATO ADDED
#cmax=np.argmax(loaded_model.predict(in_train, batch_size=batch_size, verbose =True),axis=-1)
##END
...
#New Attempt at better graphs

# Raw Data  - 5 channels 
plt.figure(figsize=(30, 6))
plt.plot(dataset[lenny:ulenny,1], color='black', label="the raw data")
plt.title("The raw test")
plt.xlabel('Time(ms)')
...
#Predicted Idealization - 5 Channels
plt.figure(figsize=(30, 6))
...
#DEEPCHANNEL - Precticted Idealization - 5 Channels
plt.figure(figsize=(30,6))
...
# how to make a graph from the internet
...
# plt.figure(figsize=(30, 6))
# arrangingGraph = np.arange(0, 3000, 1)
# ax3 = plt.subplot(211)
# ax3.plot(arrangingGraph,target_train[lenny:ulenny], color='black', label="the actual idealisation")
# plt.xlabel('timepoint')
# plt.ylabel('No. Open Channels')
